--[[
    GD50
    from the Breakout Remake https://github.com/games50/breakout
...
function EnterHighScoreState:getScoreIndex()
    for i = 1, 10 do
        if self.score > self.highScores[i].score then
            return i
        end
    end
    -- should I add a state "you suck too much to be in the high scores"
    gStateMachine:change('home')
end

function EnterHighScoreState:enter(params)
    self.highScores = HighScoreState:loadHighScore()
    self.score = params.score
    self.scoreIndex = self:getScoreIndex()
...
function EnterHighScoreState:update(dt)
    if love.keyboard.wasPressed('enter') or love.keyboard.wasPressed('return') then
        ...
        love.filesystem.write('dodge_the_creeps.lst', scoresStr)
